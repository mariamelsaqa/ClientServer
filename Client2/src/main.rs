...
fn main() 
{
    let socket = UdpSocket::bind("10.7.57.88:8081").expect("Client couldn't bind to address");
    // start timer
    let mut total_time = Duration::ZERO;
    let mut  handle = thread::spawn( || {});
    for i in 0..200000 {
        let mut data = i.to_string();
        let client_name = "-client 2".to_string();
        ...
        let now = Instant::now();
        sock.send_to(data.as_bytes(), "10.7.57.88:8084").expect("Client error sending");
