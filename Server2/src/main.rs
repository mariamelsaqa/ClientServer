...
fn main() {
     ...
     let socket = UdpSocket::bind("10.7.57.2:8087").expect("Server Could not bind socket");
...
fn distributed_election()
{
    ...
    let socket = UdpSocket::bind("10.7.57.2:8091").expect ("server Could not bind socket");// server2
    let socket2 = UdpSocket::bind("10.7.57.2:8092").expect ("server Could not bind socket");// server3
    let socket3 = UdpSocket::bind("10.7.57.2:8093").expect ("server Could not bind socket");// agent1
    let socket4 = UdpSocket::bind("10.7.57.2:8094").expect ("server Could not bind socket");// agent2
    ...
    loop {
        ...
        socket.send_to(mynum.to_string().as_bytes(),"10.7.57.71:8089").expect("error writing"); //send server 1
        socket2.send_to(mynum.to_string().as_bytes(),"10.7.57.93:8094").expect("error writing"); //send server 3
       ...
       // println!("Sent");
        ...
        //println!("server 2 ={}",mynum);
        ///println!("server 3 ={:?}",server3);


        let max = cmp::max(cmp::max(mynum,server1),server3);
        ...
        if(max == mynum)
        {
            ...
            socket3.send_to("-1".as_bytes(),"10.7.57.128:8087").expect("err");
            socket4.send_to("-1".as_bytes(),"10.7.57.88:8087").expect("err");

            socket.send_to(mynum.to_string().as_bytes(),"10.7.57.71:8089").expect("error writing"); //send server 1
            socket2.send_to(mynum.to_string().as_bytes(),"10.7.57.93:8094").expect("error writing"); //send server 3
            println!("server 2 down");
